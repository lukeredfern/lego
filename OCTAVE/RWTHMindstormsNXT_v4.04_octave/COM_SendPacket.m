...
if ~isempty(h.LastSendTime())
    ...
    while (matlab_toc(h.LastSendTime()) < (h.SendSendPause / 1000))        
...
if ~isempty(h.LastReceiveTime())
    ...
    while (matlab_toc(h.LastReceiveTime()) < (h.SendReceivePause / 1000))        
        %TODO add this pause below if CTRL+C breaking doesn't work great...
        % maybe add it only on Linux systems?
        %pause(0.001);
    end%while    
...
function USB_SendPacket_Linux(packet, h)
    ...
    ret = libusb_bulk_write(h.Handle(), sendingEndpoint, uint8(packet), length(packet), timeout);
