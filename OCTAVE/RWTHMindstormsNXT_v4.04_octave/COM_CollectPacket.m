...
%% Check if delay times are over
    ...
    if ~isempty(h.LastSendTime())
        ...
        while (matlab_toc(h.LastSendTime()) < (h.SendReceivePause / 1000))        
...
%% Retrieve data
    ...
    %[ret something reply] = calllib('libusb', 'usb_bulk_read', hNXT, receivingEndpoint, char(buffer'), length(buffer), timeout);    
     [status something reply] = libusb_bulk_read(h.Handle(), receivingEndpoint, buffer, length(buffer), timeout);
    if status < 0
